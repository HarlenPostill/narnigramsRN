# Firebase development, operations and migration

## Local emulator setup

Use Node 22 and Java 21. Ensure `java -version` reports 21 or newer on the command PATH; setting JAVA_HOME alone may leave an older Java executable first on PATH. On Homebrew macOS, prepend `/opt/homebrew/opt/openjdk@21/bin` to PATH for the emulator command. Run `npm ci` and `npm --prefix functions ci`, then `npm run emulators`. In another terminal, run `npm run web:emulator`; the helper supplies demo identifiers, disables dotenv loading and starts Expo without modifying local configuration. For an iOS emulator build, copy `.env.example` to `.env.local` and use `npm run ios`. Existing `.env` is preserved; do not overwrite it or display its values. `.firebaserc` defaults to the non-production `demo-narnigrams` project. `EXPO_PUBLIC_FIREBASE_EMULATOR_HOST` must accompany a `demo-` project ID; the repository rejects a real project ID in emulator mode.

Auth runs on 9099, Firestore on 8080, Functions on 5001, and emulator UI on 4000. The app's default callable region is `australia-southeast1`, matching Functions. A browser or iOS simulator uses `127.0.0.1`. For a physical iPhone, use the computer's reachable LAN address and explicitly configure the emulator bind host/firewall on a trusted local network. `127.0.0.1` on an iPhone is the phone itself. Never expose unauthenticated emulator ports to the public internet.

Use two independent browser profiles/incognito contexts to simulate two players; two tabs in the same profile share an auth identity. Browser developer tools can inspect console errors, callable requests and Firestore connection behavior. Disable networking and restore it to test reconciliation; do not edit server-owned Firestore records to make a UI test pass. Device testing must additionally cover background/relaunch and actual gestures.

The app uses Firebase's modular JS SDK, browser auth persistence and a native AsyncStorage persistence adapter. Account creation is explicit, required only for Ranked, and also available in Settings. This follows [Expo's Firebase integration guide](https://docs.expo.dev/guides/using-firebase/), and avoids assuming native Firebase modules work in Expo Go. Native attestation still needs a development/release build.

## Data model and access

| Collection/document | Contents | Client permission | Retention |
| --- | --- | --- | --- |
| `accountStats/{uid}` | Solo/Practice totals, best times, last 500 records | Own get; server writes | Until account deletion |
| `accountStats/{uid}/receipts` and `/migrations` | Idempotency receipts | Server only | Until account deletion |
| `players/{uid}` | Editable display name, ELO/totals, active match ID | Own document get; no writes or listing | Until account deletion |
| `queue/{uid}` | Rating snapshot, protocol/ruleset, timestamps/lease, status, result ID or AI seed | Own get; server writes | `deleteAfter`, 24 hours |
| `serverSearches/{uid_searchId}` | Cancellation receipts for in-flight search requests | None | `deleteAfter`, 24 hours |
| `serverLobby/current` | Bounded queue eligibility map, maximum 128 | None | Compact singleton; expired entries pruned |
| `matches/{id}` | Member summaries, sequence, pool/hand counts, presence, immutable result | Members get; no writes/listing | `deleteAfter`, 7 days, refreshed while active |
| `privatePlayers/{matchId}_{uid}` | Own board/hand and sequence; owner UID | Owner get; server writes | Independent `deleteAfter`, 7 days |
| `serverMatches/{id}` | Hidden pool, all authoritative ownership, replay IDs, starting games counts | None | Independent `deleteAfter`, 7 days |

Firestore rules deny all client writes; only authenticated callables mutate data through Admin SDK. Therefore input validation in Functions is part of the security boundary. No public leaderboard scan exists. The single-match subscription reads two documents and is cleaned up on unmount; the queue subscribes only to its own ticket. There is no unbounded append-only event stream. TTL field configurations live in `firestore.indexes.json`; no composite query indexes are currently needed.

`cleanup` runs every 15 minutes, selects up to 100 expired matches ordered by `cleanupAt` (latest presence plus the reconnect grace), adjudicates expired leases and prunes queue eligibility. Enable TTL on all five listed expiring collections and verify policy status after deployment. TTL is asynchronous; lease checks enforce gameplay expiry immediately without relying on deletion timing. [Firestore TTL documentation](https://firebase.google.com/docs/firestore/ttl) explains its operational behavior.

Account deletion settles an active human match as a forfeit, removes associated public/private/server match records (including both players' private match data), removes queue/search receipts/profile and deletes the Firebase Auth user. It also scans hidden matches and own private records to erase stragglers when TTL deleted a public match first. Opponents keep their aggregate rating record but lose access to that match document. This is intentional data minimization; the app does not promise permanent match history. Test partial failure/retry before release. No manual data export to a new backend is performed by this change.

## Production checklist — operator action required

1. Create/select a Firebase project and Firestore region. Set up the plan, billing and budgets required by Functions/Scheduler; confirm current service prices instead of assuming Firebase is always cheaper. Configure alerts and sensible quotas before opening traffic.
2. Register the web client configuration used by the JS SDK and enable Email/Password and Apple Authentication; disable new anonymous sign-ups. Put the public app identifiers in an ignored root `.env.local` before bundling the production app through Xcode. Remove the emulator-host variable entirely. Never put service account JSON, private keys or Admin credentials in `EXPO_PUBLIC_*` or app source.
3. Set up a least-privilege deployment identity outside the app. Deploy Functions, rules and indexes explicitly to the selected real project, after emulator checks pass. Example: `npx firebase deploy --project YOUR_FIREBASE_PROJECT_ID --only functions,firestore:rules,firestore:indexes`. This command changes live services and is not part of the local test workflow.
4. Confirm all callable endpoints and the scheduled cleanup function deployed. Confirm TTL on `queue`, `matches`, `serverMatches`, `privatePlayers`, `serverSearches`, test actual deletion, and set Cloud Logging retention. The repository's TTL file is not evidence that a console deployment succeeded.
5. Run staging two-device and abuse tests, and verify unauthorized access fails with the deployed rules. Keep review backend services available during review.
6. Publish privacy/support pages, set their HTTPS URLs for the production app, complete App Privacy and export/compliance declarations, then archive and upload the signed iOS binary through Xcode. Store operator credentials in the appropriate provider consoles, never this repository.

## Supabase retirement and credential warning

The old `.env` has been removed from the git index and preserved locally; `.env` patterns now ignore local configuration. **Rotate/revoke the old Supabase credentials and review the old project's access because those values remain in git history.** A public/anonymous key is not an Admin secret, but removing its file does not invalidate historical credentials or fix the old backend's policies. Revoke any additionally exposed privileged key immediately if the owner identifies one. No secret values have been printed or copied into these docs, and no destructive git-history rewrite was performed.

Before shutting down an existing Supabase project, determine whether real users/rating records exist, take an owner-controlled export/backup and define consent, identity mapping and a tested one-time import if preservation is required. Anonymous Supabase identities cannot simply become Firebase credentials. Validate imported display names, reject client-reported ratings, and do not silently reset existing production players. This implementation assumes a fresh Firebase identity/rating baseline for local development; production migration remains an owner decision. Retire the old API, keys and resources only after verifying any needed data migration and rollback plan.

## Apple and email account rollout

Register the native iOS app under Firebase Project settings → Your apps with bundle ID `com.hrln-interaction.narnigrams`, even though this Expo app uses the Firebase JavaScript SDK and its web client configuration. The Apple token from the native sign-in sheet targets that bundle ID; a web Services ID alone does not configure the native app. Keep the existing JS SDK environment configuration. This integration does not require switching to the native Firebase SDK or adding its plist.

Enable Email/Password and Apple in Firebase Authentication. Disable anonymous account creation after deploying these rules/functions. Existing anonymous users cannot queue until they authenticate; creating a new email or Apple account links their existing UID and preserves its server-owned rating. Signing into an already-existing account restores that account instead; ranked ratings are never merged from client data.

For Apple, enable Sign in with Apple on the app identifier `com.hrln-interaction.narnigrams` in the Apple developer account. Configure Firebase’s Apple provider with your Team ID, Key ID, private key and web Services ID where required. Register Firebase’s OAuth handler return URL and authorized web domains. Keep the Apple private key in provider configuration, never in Expo public variables. Configure Apple’s private email relay for any Firebase emails you send. See [Firebase Apple setup](https://firebase.google.com/docs/auth/web/apple) and [Expo AppleAuthentication](https://docs.expo.dev/versions/v54.0.0/sdk/apple-authentication/).

The Expo config enables `usesAppleSignIn` and the AppleAuthentication plugin. Regenerate the ignored native iOS project with `npx expo prebuild --platform ios`, install pods, then rebuild and sign in Xcode. An old installed binary cannot acquire this native module through a JavaScript refresh. Apple uses native authentication on supported iOS devices and Firebase OAuth popup on web; Android offers email/password. Test native Apple sign-in/cancellation, Hide My Email, relaunch, second-device restoration, password reset, linking a legacy session, account switching and deletion on a signed build. Provider credentials and actual Apple device behavior cannot be proven by emulator tests.

Deploy `ensurePlayer`, `matchmaking`, `matchCommand`, `syncStats`, `migrateStats`, `revokeApple`, `deleteAccount`, cleanup and the Firestore rules together. Callables and rules reject anonymous tokens. Apple deletion reauthenticates and revokes the Apple grant before deleting Firebase identity/data. For native revocation, provision the `APPLE_SIGN_IN_CONFIG` Functions secret as JSON containing `teamId`, `keyId`, `privateKey` (the Apple .p8 PEM) and `clientId` (`com.hrln-interaction.narnigrams`). Use Firebase Secret Manager (`firebase functions:secrets:set APPLE_SIGN_IN_CONFIG`) without committing the value. Web revocation uses Firebase’s configured Apple provider. Tokens and authorization codes are never retained. Test revocation against Apple on a signed device; the Auth emulator cannot prove it.

Solo/Practice guest play stays local. The first account on an installation claims its existing offline history; later account switches do not re-import it. Signed-in offline results use a UID-scoped durable outbox and retry every 15 seconds while the app runs. Import receipts prevent double counting across retries; archived totals and best times are preserved beyond the 500-record local limit. Client-submitted offline stats can never modify ranked rating/wins. Ranked games, wins, losses and peak rating are maintained by server settlement. Old locally fabricated/bot ELO is not imported into competitive ratings.

## September 2026 connection repair

The production logs showed Cloud Run rejecting Firebase ID tokens before callable execution. Callable endpoints explicitly use `invoker: "public"`: this lets requests reach the Firebase callable middleware, which verifies Firebase Auth tokens. Every handler still requires an Apple or password identity; Firestore remains private with server-only writes. Do not apply public invocation to the scheduled cleanup function. See [callable authentication](https://firebase.google.com/docs/functions/callable) and [Cloud Run invocation](https://docs.cloud.google.com/run/docs/authenticating/overview).

Use the existing `(default)` Firestore Native database in `narms-7c6bc`. Realtime Database is not needed: this app uses Firestore snapshot listeners plus server heartbeats and leases. Keep all client Firebase identifiers from the same project and `EXPO_PUBLIC_FIREBASE_FUNCTIONS_REGION=australia-southeast1`; omit emulator host in production. A Firebase 401 should be investigated in Functions/Cloud Run logs before changing database rules.

Deploy the current callable handlers, including `updateProfile`, `syncStats` and `migrateStats`, as well as rules/indexes. `updateProfile` accepts only a normalized public display name (1–30 Unicode characters, no control/format characters). It never accepts ratings or account IDs from the client. Duplicate names are supported; active matches retain the name snapshot from their creation.

Native Apple deletion also needs the separate `APPLE_SIGN_IN_CONFIG` secret and `revokeApple` deployment, as described above. Do not create placeholder credentials. Missing revocation setup does not prevent Apple login or matchmaking.

After updating the app, test on two signed-in devices: save names, queue together, verify opponent names, cancel a search, retry after network loss, and reopen a match. If cancellation cannot reach the server, the UI leaves within two seconds; an existing match remains recoverable on Play and an unclaimed queue entry expires through its lease.

### Verified deployment on 2026-09-19

Deployed Sydney `ensurePlayer`, `matchmaking`, `matchCommand`, `deleteAccount`, `updateProfile`, `syncStats`, `migrateStats`, Firestore rules and field configurations. Firebase CLI validated the missing Apple secret even with a function filter, so this deployment used a temporary copy of the compiled backend with only the `revokeApple` export omitted. The repository retains the full implementation; provision the secret before the next normal full deployment. No existing functions were deleted. All five `deleteAfter` TTL policies were verified ACTIVE.

The CLI created correct invocation bindings for new endpoints but did not repair missing bindings on the existing endpoints. The Sydney callable services now have `allUsers` in `roles/run.invoker`; unrelated policy bindings were retained. Cleanup invocation was not opened. A disposable production email account successfully called `ensurePlayer`, `updateProfile` and `syncStats`, then was removed through `deleteAccount`. Unauthenticated requests return the callable JSON sign-in rejection. Native Apple sign-in and two-device play still require device verification.

Local checks passed: lint (existing layout import warnings only), TypeScript, domain/backend tests, and all emulator tests. Browser checks covered account details, connection check, saving a name, queue identity, successful cancellation, and retry/back navigation with the emulator offline. The emulator launcher now clears Metro cache, and Metro excludes dotenv files when `EXPO_NO_DOTENV=1` because Expo's development virtual env loader otherwise merges them back into the bundle. `.env.example` uses the required `demo-` project ID.
