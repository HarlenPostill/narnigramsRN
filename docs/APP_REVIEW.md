...
| **Needs review — 1.2** editable display names | Account settings now allow non-unique public names through a validated server callable; no public boards/chat | Add name moderation, report/block and an operational response process before release. Input length/control-character validation is not content moderation. |
