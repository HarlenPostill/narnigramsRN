...
- Confirm bundle ID `com.hrln-interaction.narnigrams`, Apple team and application ownership. `eas.json` contains no invented team/project IDs. Link the Expo project and configure signing in the owner's account.
- Run the `simulator` EAS profile for a simulator binary, `preview` for internal device distribution, and `production` for the signed store build. These profiles are normal binaries; no development-client dependency is assumed. See [Expo build profiles](https://docs.expo.dev/build/eas-json/) for the configuration model. Do not confuse a successful web export with an iOS release build.
